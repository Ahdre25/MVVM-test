...
class BaseTarget<T1: Encodable, T2: Decodable>: TargetType {
    ...
    var path: String = ""
    let baseURL: URL = URL(string: "https:/iqtest.port-tranzit.ru/api/")!
//    let baseURL: URL = URL(string: "https://iq.port-tranzit.ru/api/")!
    var method: Moya.Method = .get
    let sampleData: Data = Data()
    var headers: [String : String]? {
        return ["Authorization": token, "ClientType": "iOS"]
    }
    var task: Task {
    ...
    func request() {
        ...
        let provider = MoyaProvider<Self>(plugins: [NetworkLoggerPlugin(configuration: NetworkLoggerPlugin.Configuration(logOptions: .verbose))])
...
//                            self.model.view?.showToast("Ошибка парсинга")
                ...
                if response.statusCode == 429 {
                    if self.model.showToast {
//                        self.model.toastView?.showErrorToast("Превышен лимит запросов. Повторите попытку позднее")
                    }
...
//                        self.model.view?.loader(false)
//                        self.model.view?.showErrorToast("При загрузке произошла ошибка. Проверьте интернет-соединение")
                    } else {
                        print("\(error.errorDescription ?? "")")
//                        self.model.view?.showErrorToast("\(error.errorDescription ?? "неизвестная ошибка")")
                    }
                }
